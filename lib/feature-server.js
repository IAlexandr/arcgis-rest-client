  ...
  addAttachment(props) {
    ...
    return this.checkToken(params)
      .then((params) => {
        new Promise((resolve) => {
          // request
          //   .post(this.featureServerUrl + '/' + props.objId + '/addAttachment')
          //   .type('form')
          //   .accept('application/json')
          //   .send(params)
          //   .attach('attachment', props.filePath)
          //   .end((err, res) => {
          ...
          const rs = fs.createReadStream(props.filePath);
          const url = this.featureServerUrl + '/' + props.objId + '/addAttachment';

          var r = atRequest.post(url, function (err, resp, body) {
            if (err) {
              throw err;
            }
            resolve(body);
